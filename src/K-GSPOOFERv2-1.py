...
def makeID(a,b): # True stays for GUID, false for HWID 
    ...
    if not b:
        ...
        id = id.replace('{','') # Quit that symbol bc the HWID doesn't have it
    return id

# Make an entry log that records the date, the old and the new ids.
...
def main():
    ...
    while run == 1:
        ...
        if not ctypes.windll.shell32.IsUserAnAdmin():
            ...
            print(bcolors.blue+"                                                                              github.com/dev-dimitrov"+bcolors.end)
